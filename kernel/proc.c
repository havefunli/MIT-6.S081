...
void userinit(void)
{
  ...
  pagecopy(p->pagetable, p->kernel_pageta, 0, p->sz);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    if (pagecopy(p->pagetable, p->kernel_pageta, p->sz, sz) != 0) {
    ...
    if (sz != p->sz) {
      ...
      uvmunmap(p->kernel_pageta, PGROUNDUP(sz), (PGROUNDUP(p->sz) - PGROUNDUP(sz)) / PGSIZE, 0);
    }
  }
  p->sz = sz;
  return 0;
...
int fork(void)
{
  ...
  if (pagecopy(np->pagetable, np->kernel_pageta, 0, np->sz) != 0)
  {
    freeproc(np);
    release(&np->lock);
    return -1;
